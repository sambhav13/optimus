...
// GetReplayTaskConfigByScheduledAt provides a mock function with given fields: scheduledAt
func (_m *ReplayRepository) GetReplayTaskConfigByScheduledAt(scheduledAt time.Time) (map[string]string, error) {
	ret := _m.Called(scheduledAt)

	var r0 map[string]string
	var r1 error
	if rf, ok := ret.Get(0).(func(time.Time) (map[string]string, error)); ok {
		return rf(scheduledAt)
	}
	if rf, ok := ret.Get(0).(func(time.Time) map[string]string); ok {
		r0 = rf(scheduledAt)
	} else {
		if ret.Get(0) != nil {
